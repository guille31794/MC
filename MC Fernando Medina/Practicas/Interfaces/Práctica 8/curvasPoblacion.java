	    ...
	    g.drawString(" = Vivas",1140,112);
	     ...
	     g.drawString("N",1180,525);
	     	...
	     	for(int j=0;j<puntos.length-1;j++){
		     	g.setColor(Color.RED);
		     	g.drawLine(50 +(j),500-puntos[j]*405/(dimension*dimension),50 +(j+1),500-puntos[j+1]*405/(dimension*dimension));

		     try{Thread.sleep(2);}catch(InterruptedException popo){}
	     	}//for j
	}//constructor
}//clase
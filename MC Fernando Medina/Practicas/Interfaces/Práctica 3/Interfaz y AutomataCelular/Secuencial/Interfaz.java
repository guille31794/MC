...
class LaminaTrabajoGeneral extends JPanel implements ActionListener //,MouseListener
{
	...
	public LaminaTrabajoGeneral()
	{
		...
		Ejecutar.setBounds(53,90,200,200);
		...
		//laminaGrafica.setBackground(Color.BLUE);
		...
		//LaminaProgramas
		laminaProgramas.setLayout(new GridLayout(5,2,0,2));
		...
		//laminaBotones.addMouseListener(this);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==Ejecutar)
		{
			...
			{
				JOptionPane.showMessageDialog(null,"La VECINDAD debe estar entre 1-899","ERROR",JOptionPane.ERROR_MESSAGE);
			}

			else if(regla>(Math.pow(ST,(2*GV+1)*(ST-1)+1)-1))
			{
				JOptionPane.showMessageDialog(null,"Regla no compatible con estados y vecindad introducidos","ERROR",JOptionPane.WARNING_MESSAGE);
			}
			...
			else
			{
				...
				laminaGrafica.iteracion();

				laminaGrafica.flag = true;
				Graphics g = laminaGrafica.getGraphics();
				laminaGrafica.paint(g);
		...
		numeroAleatorio = new randomGenerator();
			switch(g)
			{
				case 0: 
						...
						laminaGrafica.NoRandom();

				break;

				case 1: 
						
						laminaGrafica.Random(numeroAleatorio.generador1(ST));

				break;
				case 2: 
						laminaGrafica.Random(numeroAleatorio.generador2(ST));
				break;
				case 3: 
					
						laminaGrafica.Random(numeroAleatorio.generador3(ST));
				break;
				case 4: 
					
						laminaGrafica.Random(numeroAleatorio.generador4(ST));

				break;
				case 5: 
						
						laminaGrafica.Random(numeroAleatorio.generadorCombinado(ST));

				break;
				case 6: 
						
						laminaGrafica.Random(numeroAleatorio.FishmanYMoore1(ST));

				break;
				case 7: 
					
						laminaGrafica.Random(numeroAleatorio.FishmanYMoore2(ST));
				break;
				case 8: 

						laminaGrafica.Random(numeroAleatorio.RANDU(ST));
				break;
			}//switch
...
/*	public void mouseEntered(MouseEvent ev)
	{
		Ejecutar = new JButton(new ImageIcon("ejecutar2.png"));
